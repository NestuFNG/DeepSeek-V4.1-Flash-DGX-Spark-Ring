...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        # Tech2Wild/Kai 2026-09-10 (DGX Spark / SM12x): persistent_topk plans a
        # grid that must be co-resident (90 CTAs for TopK=512) but a GB10 has 48
        # SMs and ~99 KB smem/block, so launch_persistent_topk raises
        # "persistent_topk would oversubscribe and the FilteredTopK fallback
        # requires >=128KB smem per block". Take the generic per-row path there,
        # like the cooperative kernel gate above already does.
        use_persistent_topk = (
            current_platform.is_cuda()
            and topk_tokens in (512, 1024, 2048)
            and not current_platform.is_device_capability_family(120)
