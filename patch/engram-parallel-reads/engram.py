...
class DiskEngramTable:
    ...
    def _submit_rows(self, fd: int, base: int, rel: list, row_bytes: int, buf) -> list:
        """Start the row reads and return their futures ([] if done inline). Small
        decode batches are spread over the pool too: at decode each layer needs only
        ~6 rows per rank, and serial preads cost ~1 ms each over NFS."""
        ...
        n = len(rel)
        chunk = max(1, min(self.chunk, -(-n // self.threads)))
        if n <= 1:
            work(0, n)
            return []
        return [self.pool.submit(work, lo, min(lo + chunk, n)) for lo in range(0, n, chunk)]

    def _read_rows(self, fd: int, base: int, rel: list, row_bytes: int, buf) -> None:
        for fut in self._submit_rows(fd, base, rel, row_bytes, buf):
            fut.result()
    ...
    def gather_dequant(self, rel: torch.Tensor, owned: torch.Tensor) -> torch.Tensor:
        ...
        rel_l = rel.tolist()
        futs = self._submit_rows(self.w_fd, self.w_off, rel_l, self.dim, memoryview(w.numpy()).cast("B"))
        futs += self._submit_rows(self.s_fd, self.s_off, rel_l, self.sb, memoryview(s.numpy()).cast("B"))
        for fut in futs:
            fut.result()
