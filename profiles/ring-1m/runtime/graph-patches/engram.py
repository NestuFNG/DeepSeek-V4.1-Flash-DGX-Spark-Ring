# Modified for the DGX Spark Ring profile, 2026-09-11; see repository root NOTICE.md.
...
# SSD table reader adapted from Tech2Wild/Kai; preserve official DP/ubatch logic.
...
def gather_dequant_many(requests: list) -> list[torch.Tensor]:
    ...
    for table, rel, owned in requests:
        if rel.device.type != "cpu" or owned.device.type != "cpu":
            raise ValueError("Engram disk row indices and ownership must be on CPU")
        uniq, inverse = torch.unique(rel, return_inverse=True)
        if uniq.numel() and (int(uniq[0]) < 0 or int(uniq[-1]) >= table.num_rows):
            raise IndexError("Engram row outside the backing table")
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def disk_rel_owned(self, local: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """CPU global row ids into a full local table, with non-owned rows masked."""
        if local.device.type != "cpu":
            raise ValueError("disk_rel_owned requires CPU indices")
        if local.shape[1] < self.part_n_hash_cols:
            pad = torch.full((local.shape[0], self.part_n_hash_cols-local.shape[1]),
        ...
        # Our DiskEngramTable opens the FULL file at row_start=0 on every rank.
    ...
    def _disk_lookup(self, indices: torch.Tensor, out: torch.Tensor) -> None:
        if torch.cuda.is_current_stream_capturing():
            raise RuntimeError("SSD Engram requires the V2 prestage hook for CUDA Graph")
        count = indices.shape[0]
        end = min(self.head_start + self.part_n_hash_cols, self.n_hash_cols)
        local = indices[:, self.head_start:end].detach().to("cpu", dtype=torch.int64)
        rel, owned = self.disk_rel_owned(local)
        assert self.disk is not None
        values = self.disk.gather_dequant(rel, owned)
        out[:count].copy_(values.view(count, self.part_n_hash_cols, self.dim))
...
class Engram(nn.Module):
    ...
    def __init__(
        ...
        # Port the community graph-safe SSD staging onto official 0909 without
        # replacing its DP/ubatch implementation. This optimization is DP1 only.
        self.prestage = self.embed_tokens.disk is not None and bool(
            getattr(vllm_config, "use_v2_model_runner", False)
        )
        if self.prestage and (self.embed_tokens.dp_size != 1 or
                              vllm_config.parallel_config.use_ubatching):
            raise ValueError("SSD Engram graph prestaging currently requires DP1 and one ubatch")
    ...
    def prepare_embeddings(self, hash_ids: torch.Tensor) -> None:
        ...
        """
        if self.prestage:
            return  # ModelState has staged the actual rows before this forward.
...
class EngramDiskStager:
    ...
    def stage(
        ...
        if n > self.max_tokens:
            raise ValueError("Engram prestage exceeds the persistent buffer")
