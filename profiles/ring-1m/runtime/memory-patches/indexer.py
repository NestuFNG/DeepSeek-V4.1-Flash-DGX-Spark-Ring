...
def get_max_prefill_buffer_size(vllm_config: VllmConfig):
    ...
    # The gather buffer only needs the contexts of simultaneously scheduled
    # requests. Preserve the upstream cap for other models and large batches.
    # Metadata and the indexer operator both call this function, so allocation
    # and chunking use the same bound. No KV values or arithmetic are changed.
    model_type = getattr(vllm_config.model_config.hf_config, "model_type", "")
    if model_type in ("deepseek_v41", "deepseek_v41_text"):
        request_bound = max(1, min(40, vllm_config.scheduler_config.max_num_seqs))
        return max_model_len * request_bound
