...
class DiskEngramTable:
    ...
    def gather_dequant(self, rel: torch.Tensor, owned: torch.Tensor) -> torch.Tensor:
        """rel: [R] global row IDs into the full on-disk table; output is CPU bf16."""
        R = rel.numel()
        w = torch.empty((R, self.dim), dtype=torch.uint8, device="cpu")
        s = torch.empty((R, self.sb), dtype=torch.uint8, device="cpu")
        rel_l = rel.tolist()
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def _disk_lookup(self, indices: torch.Tensor, out: torch.Tensor) -> None:
        ...
        if local.shape[1] < L:
            pad = torch.full((T, L - local.shape[1]), -1, dtype=torch.int64, device="cpu")
            local = torch.cat([local, pad], dim=1)
        rows = local.reshape(-1)
        owned = (rows >= self.vocab_start_idx) & (rows < self.vocab_end_idx)
        # The backing safetensors contains the FULL table; address global rows.
