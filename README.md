...
**长代码保留未完成项。** 每路原始输出额度为 **32,768 tokens（含思考）**，实际输出 17,184–32,768，总输出中 153,323 为思考 token。六路自然结束，通过 Python 语法、五个必需函数、至少二十个测试方法定义及项目标记检查；**未执行生成代码或它生成的测试**。两路因输出额度耗尽而截断，不能标成八路代码验收通过。两条 65,536-token 额度重试另行记录，不混入本表；最终候选仍需重新通过完整八路长输出测试。
...
| 工况 | 上下文与负载 | 当前状态 |
|---|---|---|
| 八路长代码复验 | 每路约 500K，输出额度提高到 65,536，仍为 ON/max | 原始批次两条截断请求正在单独重试；完整八路复验待完成 |
