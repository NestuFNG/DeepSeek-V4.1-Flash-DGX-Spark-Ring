#!/bin/bash
# postcheck10.sh (root on Reddie) after boot 10 serves: where each rank reads Engram from, the KV pool,
# then verify9.py (count-to-100 x2, code x2, one tool call, one tiny image).
...
O=/var/tmp/boot-results/boot10; mkdir -p $O
